...
class SqliteHandler(object):
    ...
    def insert_identity_and_dna_algns_columns(
        ...
        with sqlite3.connect(self.results_database_path, timeout=self.timeout_database) as db:
            ...
            if self.check_if_column_in_table_exists(
                ...
                cursor.execute(""" ALTER TABLE Matches DROP COLUMN dna_perc_identity;""")
            if self.check_if_column_in_table_exists(
                    table_name='Matches',
                    column_name='prot_perc_identity',
            ):
                cursor.execute(""" ALTER TABLE Matches DROP COLUMN prot_perc_identity;""")
            if self.check_if_column_in_table_exists(
                    table_name='Matches',
                    column_name='query_dna_seq',
            ):
                cursor.execute(""" ALTER TABLE Matches DROP COLUMN query_dna_seq;""")
            if self.check_if_column_in_table_exists(
                    table_name='Matches',
                    column_name='target_dna_seq',
            ):
                cursor.execute(""" ALTER TABLE Matches DROP COLUMN target_dna_seq;""")
            cursor.execute(""" ALTER TABLE Matches ADD COLUMN dna_perc_identity REAL;""")
            cursor.execute(""" ALTER TABLE Matches ADD COLUMN prot_perc_identity REAL;""")
            cursor.execute(""" ALTER TABLE Matches ADD COLUMN query_dna_seq VARCHAR;""")
            cursor.execute(""" ALTER TABLE Matches ADD COLUMN target_dna_seq VARCHAR;""")
            cursor.executemany(""" 
            UPDATE Matches 
            SET 
                dna_perc_identity=?,
    ...
    def insert_event_categ_full_length_events_cumulative_counts(
        ...
        with sqlite3.connect(self.results_database_path, timeout=self.timeout_database) as db:
            cursor = db.cursor()
            if self.check_if_column_in_table_exists(
                    table_name='Transcript_classification_counts',
    ...
    def insert_percent_query_column_to_fragments(
        ...
        with sqlite3.connect(self.results_database_path, timeout=self.timeout_database) as db:
            ...
            if self.check_if_column_in_table_exists(
                ...
                cursor.execute("""DROP VIEW IF EXISTS Full_length_matches;""")
                cursor.execute(""" ALTER TABLE Matches DROP COLUMN percent_query;""")
            cursor.execute("""
