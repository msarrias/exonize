...
class Exonize(object):
    def __init__(
        ...
        self.dp = DataPreprocessor(self.log.logger, **self.__dict__)
    ...
    def align_CDS(self, gene_id: str, query_seq: str, hit_seq: str, query_coord: P.Interval, cds_frame: str) -> dict:
        ...
        def tblastx_with_saved_io(
            ...
            """
            output_file = os.path.join(self.dp.working_dir, f'output/{ident}_output.xml')
            if not os.path.exists(output_file):
                query_filename = os.path.join(self.dp.working_dir, f'input/{ident}_query.fa')
                target_filename = os.path.join(self.dp.working_dir, f'input/{gene_id_}_target.fa')
        ...
        def execute_tblastx_using_tempfiles(
            ...
            """
            with tempfile.TemporaryDirectory(dir=self.dp.working_dir) as tmpdirname:
        ...
        chrom = self.dp.gene_hierarchy_dict[gene_id]['chrom']
        gene_coord = self.dp.gene_hierarchy_dict[gene_id]['coord']
    ...
    def get_gene_tuple(
        ...
        gene_coord = self.dp.gene_hierarchy_dict[gene_id]['coord']
        return (gene_id,
                self.dp.gene_hierarchy_dict[gene_id]['chrom'],
                self.dp.gene_hierarchy_dict[gene_id]['strand'],
                gene_coord.lower,
                gene_coord.upper,
                has_dup_bin)
    ...
    def get_candidate_CDS_coords(
        ...
        cds_coords_and_frames = list(set(
            ...
            for mrna_annotation in self.dp.gene_hierarchy_dict[gene_id]['mRNAs'].values()
    ...
    def find_coding_exon_duplicates(
        ...
        def check_for_masking(
            ...
            try:
                ...
                if masking_perc > self.masking_perc_threshold:
                    seq_ = ''
                    if type_ == 'gene':
                        self.log.logger.file_only_info(
        ...
        chrom, gene_coord, gene_strand = (self.dp.gene_hierarchy_dict[gene_id]['chrom'],
                                          self.dp.gene_hierarchy_dict[gene_id]['coord'],
                                          self.dp.gene_hierarchy_dict[gene_id]['strand'])
        temp_gs = str(Seq(self.dp.genome[chrom][gene_coord.lower:gene_coord.upper]))
        ...
        if gene_seq:
            ...
            if CDS_coords_dict:
                for cds_coord in CDS_coords_dict['set_coords']:
                    temp_cds = str(Seq(self.dp.genome[chrom][cds_coord.lower:cds_coord.upper]))
    ...
    def identify_full_length_duplications(self) -> None:
        ...
        for row in rows:
            ...
            for mrna, trans_dict in self.dp.gene_hierarchy_dict[gene_id]['mRNAs'].items():
    ...
    def assign_event_ids(self, full_matches_list: list[tuple]) -> (list[tuple], set[tuple]):
        ...
        for gene_id, records in full_matches_dict.items():
            gene_start = self.dp.gene_hierarchy_dict[gene_id]['coord'].lower
    ...
    def get_identity_and_dna_seq_tuples(self) -> list[tuple]:
        ...
        def fetch_dna_sequence(chrom: str,
            ...
            """
            seq = Seq(self.dp.genome[chrom][annot_start:annot_end][pos_annot_s:pos_annot_e])
    ...
    def run_exonize_pipeline(self) -> None:
        ...
        self.dp.prepare_data()
        gene_ids = list(self.dp.gene_hierarchy_dict.keys())
        ...
        if unprocessed_gene_ids:
            ...
            self.log.logger.info(
                f'Starting exon duplication search for {len(unprocessed_gene_ids)}/{gene_n} genes.'
            )
        ...
        insert_event_categ_full_length_events_cumulative_counts(
            self.results_db, self.timeout_db, reduced_event_types_tuples
        )
        identity_and_sequence_tuples = self.get_identity_and_dna_seq_tuples()
        insert_identity_and_dna_algns_columns(
            self.results_db, self.timeout_db, identity_and_sequence_tuples
        )
        ...
        insert_event_id_column_to_full_length_events_cumulative_counts(
            self.results_db, self.timeout_db, fragments_tuples
        )
